...
import mt.spring.core.fragment.*;
...
public class RedisTaskFragment implements TaskFragment {
	...
	public RedisTaskFragment(@NotNull String scheduleName, @NotNull RedisTemplate<String, Object> redisTemplate) {
		this(scheduleName, redisTemplate, getHostIp(null));
	}
	
	public RedisTaskFragment(@NotNull String scheduleName, @NotNull RedisTemplate<String, Object> redisTemplate, @NotNull String currentInstanceId) {
	...
	private void deleteExpires() {
		list().stream().map(o -> o + "").filter(this::isItemExpired).forEach(s -> redisTemplate.opsForZSet().remove(getKey(), s));
	}

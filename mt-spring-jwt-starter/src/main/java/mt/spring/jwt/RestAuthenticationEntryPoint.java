...
import static mt.spring.jwt.JwtAuthenticationTokenFilter.ATTR_IS_TOKEN_PASS;
...
public class RestAuthenticationEntryPoint extends LoginUrlAuthenticationEntryPoint {
	...
	public void commence(HttpServletRequest request, HttpServletResponse response, AuthenticationException authException) throws IOException, ServletException {
		...
		Object isTokenPass = request.getAttribute(ATTR_IS_TOKEN_PASS);
		if (isToken == null || !Boolean.parseBoolean(isToken.toString())) {
			...
			int status;
			if (isTokenPass != null && "true".equals(isTokenPass.toString())) {
				status = 403;
				resResult.setMessage("无权限访问");
			} else {
				status = 401;
			}
